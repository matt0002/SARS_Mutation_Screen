...
def analysis_on_POI():
    ...
    for seq in SeqIO.parse('Whole_genome_with_B.1.1.7_patients_of_interest.fasta', format='fasta'):
        ...
        for seq_2 in SeqIO.parse('Whole_genome_with_B.1.1.7_patients_of_interest.fasta', format='fasta'):
            ...
            # ORF1ab is actually from 266 to 21555 But did 100bp upstream and downstream region for error
            for seq_bases, seq_2_bases in zip(seq.seq[166:21655],
                                              seq_2.seq[166:21655]):
                print(len(seq))
    ...
    df.to_csv('Heatmap_data_for_ORF1ab_.csv')


def graph_heatmap():
    df = pd.DataFrame(pd.read_csv('Heatmap_data_for_ORF1ab_.csv'))
    ...
    plt.show()
    plt.savefig('ORF1ab_DNA_heatmap.svg', format='svg')
...
def compare_to_wt():
    ...
    for seq in SeqIO.parse('Whole_genome_with_B.1.1.7_patients_of_interest.fasta', format='fasta'):
        if seq.id == "B.1.1.7":
            wt = seq.seq
    wt = wt[166:21655]  # todo ask nuno about this....
    matrix_of_bp = {}
    for wt_index, wt_element in enumerate(wt):
        ...
        for seq_ in SeqIO.parse('Whole_genome_with_B.1.1.7_patients_of_interest.fasta', format='fasta'):
            seq_trimmed = seq_.seq[166:21655]
    ...
    df_1.to_csv('ORF1ab_mutation_0.1_compared_to_B.1.1.7.csv')
    df_2.to_csv('ORF1ab_mutation_0.2_compared_to_B.1.1.7.csv')


def make_hist():
    df_1 = pd.read_csv('ORF1ab_mutation_0.1_compared_to_B.1.1.7.csv')
    df_2 = pd.read_csv('ORF1ab_mutation_0.2_compared_to_B.1.1.7.csv')
    ...
    hist_1.write_html('ORF1ab_mutations_to_B.1.1.7.html')
